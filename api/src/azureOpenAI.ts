...
export async function azurePing(cfg: AzureChatConfig): Promise<void> {
  if (process.env.FAKE_LLM === '1') return;
  ...
  if (content.trim() !== 'OK') {
    // Not fatal, but indicates misconfiguration/deployment not chat-capable.
    throw new Error(`Azure OpenAI test call succeeded but returned unexpected content: ${JSON.stringify(content)}`);
  }
}
